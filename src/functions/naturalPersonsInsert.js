...
const mapDataToInsertManyNaturalPersons = (data) => {
  const gisMapped = data.map((gi) => {
    return {
      ...
      contrato_faenas: [],
      imagen_perfil_gi: '',
      licencia_conduccion: gi.licencia_conduccion || 'No',
      clase_licencia: !!gi.clase_licencia ? [gi.clase_licencia] : [],
      fecha_venc_licencia: moment(gi.fecha_venc_licencia, FORMAT_DATE).format(FORMAT_DATE) || '',
      estado_licencia: gi.estado_licencia,
      nivel_educacional: '',
      rol: 'Clientes',
      ...
      genero: gi.genero || '',
      grupo_sanguineo: gi.grupo_sanguineo || '',
      usa_lente_optico: gi.usa_lente_optico || '',
      usa_lente_contacto: gi.usa_lente_contacto || '',
      usa_audifonos: gi.usa_audifonos || '',
