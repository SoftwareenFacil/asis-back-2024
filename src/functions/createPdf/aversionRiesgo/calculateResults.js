...
export const getResultsTest = results => {
    return results.length > 0 && results.map(e => {
        if (e.result >= 2 && e.result <= 3.1) return 'bajo';
        if (e.result > 3.1 && e.result <= 4.367) return 'promedio';
...
export const getResultTest = number => {
    if (number >= 2 && number <= 3.1) return 'Bajo';
    if (number > 3.1 && number <= 4.367) return 'Promedio';
...
export const testResult = number => {
    if (number >= 2 && number <= 3.1) return 1;
    if (number > 3.1 && number <= 4.367) return 2;
